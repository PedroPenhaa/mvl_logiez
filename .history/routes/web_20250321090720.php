...
// Middleware temporário para autenticação enquanto não temos banco de dados
Route::middleware(['web'])->group(function () {
    Route::get('/dashboard', [HomeController::class, 'index'])
        ->middleware(function (Request $request, Closure $next) {
            // Verifica se usuário está "autenticado" pela sessão temporária
    ...
    // Rotas da API para carregamento de seções (AJAX) - protegidas
    Route::prefix('api')->name('api.')->middleware(function (Request $request, Closure $next) {
        // Verifica se usuário está "autenticado" pela sessão temporária
        if (!session('authenticated')) {
