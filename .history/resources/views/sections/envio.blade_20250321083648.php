        ...
        <form id="envio-form" action="{{ route('api.envio.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    <h5 class="mb-3">Dados do Remetente</h5>
                    <div class="mb-3">
                ...
                <div class="col-md-6 mb-4">
                    <h5 class="mb-3">Dados do Destinatário</h5>
                    <div class="mb-3">
            ...
            <div class="row mt-3">
                <div class="col-12">
                    <h5 class="mb-3">Dados da Mercadoria</h5>
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="tipo_envio" name="tipo_envio" required>
                                        ...
                                        <option value="amostra">Amostra Comercial</option>
                                        <option value="presente">Presente</option>
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="valor_mercadoria" class="form-label">Valor da Mercadoria (USD)</label>
                                    <input type="number" step="0.01" min="0.01" class="form-control" id="valor_mercadoria" name="valor_mercadoria" required>
                                </div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="descricao_mercadoria" class="form-label">Descrição da Mercadoria</label>
                                <textarea class="form-control" id="descricao_mercadoria" name="descricao_mercadoria" rows="3" required></textarea>
                            </div>
                            
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="item_liquido" name="item_liquido">
                                    <label class="form-check-label" for="item_liquido">
                                        Contém itens líquidos
                                    </label>
                                </div>
                                <div id="liquido_info" class="mt-2 p-2 bg-light border rounded d-none">
                                    <p class="mb-1"><strong>Importante:</strong> Envios contendo líquidos precisam de declaração especial.</p>
                                    <p class="mb-0 small">Será necessário preencher a ficha de declaração de produtos não perigosos.</p>
                                </div>
                            </div>
            ...
            <div class="text-center">
                <button type="submit" class="btn btn-primary">Prosseguir para Pagamento</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        // Controle de exibição de campos conforme tipo de pessoa
        ...
        // Controle de exibição de informações de item líquido
        $('#item_liquido').change(function() {
            if ($(this).is(':checked')) {
                $('#liquido_info').removeClass('d-none');
            } else {
                $('#liquido_info').addClass('d-none');
            }
        });
        
        // Processar envio via AJAX
        $('#envio-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Ir para a seção de pagamento
                    $('.menu-item[data-section="pagamento"]').click();
                },
                error: function() {
                    // Mostrar mensagem de erro
                    alert('Erro ao processar o envio. Verifique os dados e tente novamente.');
                }
            });
        });
    });
